...
public class FixedLengthRecordReader  extends RecordReader<LongWritable, Text> {
	...
	public boolean nextKeyValue() throws IOException, InterruptedException {
		...
		if (result == -1) {
			...
			byte[] shortRecord = new byte[result];
			System.arraycopy(currentRecord, 0, shortRecord, 0, result);
			key.set(result);
			value.set(shortRecord);
			System.out.println("Returning Short Record:" + result + " " + shortRecord.length);
			return true;
		} else {
			key.set(result);
